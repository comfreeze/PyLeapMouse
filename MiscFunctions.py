...
class NStateDebouncer(object):  # A signal debouncer that has `number_of_states` states
    ...
    def signal(self, signal_value):
        ...
        self.state_counters[signal_value] += 1  # Increment signalled state
        for i in range(0, len(self.state_counters)):
            if i is not signal_value:
                self.state_counters[i] -= 1  # Decrement all others
        for i in range(0, len(self.state_counters)):  # Fix bounds and check for a confirmed state change
            if self.state_counters[i] < 0:
                self.state_counters[i] = 0
            if self.state_counters[i] >= self.debounce_time:  # Confirmed new state at index i
                ...
                for x in range(0, len(self.state_counters)):
                    if x is not i:
                        self.state_counters[x] = 0  # Zero out all other state counters
                self.state = i  # Save the new state
        return self.state


class FingerTools(object):
...
def has_thumb(hand):  # The level of accuracy with this function is surprisingly high
    ...
    # Check if the finger closest to the palm is more than 20mm closer than the average distance
    if average - minimum > 20:
